...
# SEO Audit Tool – v3 (contributi pesati) — titolo fisso + icona
# Avvio: streamlit run seo_audit_tool_v3_full.py
# Requisiti: streamlit pandas numpy openpyxl matplotlib
...
APP_TITLE = "SEO Audit Tool"

# ============= Matplotlib setup (radar) =============
def setup_matplotlib():
    try:
        ...
        st.warning("Modulo 'matplotlib' mancante. Esegui: pip install matplotlib")
        return False, None
...
def estrai_kpi(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Immagini senza ALT -> % pagine con >=1 immagine senza ALT
    img_pages_pct = None
    ...
    # CORE WEB VITALS (da BM/BO/BR/CO/DC)
    cwv_vals = {
...
with tab2:
    ...
    if files:
        ...
        if risultati:
            ...
            # ======= Benchmark competitivo: valori + contributi pesati =======
            ...
            # Form per input DR/LV con submit e scala
            if "manual_ext_df" not in st.session_state:
                st.session_state.manual_ext_df = pd.DataFrame({
                    "Dominio": df_riepilogo["Dominio"].tolist(),
                    "DR": [None] * len(df_riepilogo),
                    "LINK VELOCITY": [None] * len(df_riepilogo),
                })
            ...
            with st.form("offpage_form", clear_on_submit=False):
                ...
                scale = st.selectbox("Scala valori DR/LINK VELOCITY", ["0-100", "0-1"], index=0, help="Se fornisci valori 0–1 verranno moltiplicati *100.")
            ...
            # Contributi pesati per riga (value% * weight)
            ...
            bench_vals.loc["SCORE"] = score  # per riferimento
            bench_pts.loc["SCORE"] = score   # identico per definizione

            # Tabella combinata con MultiIndex (Domain, Value% / Pts)
            tuples = []
            for c in cols:
            ...
            st.caption("Ogni riga mostra **valore (%)** e **contributo (Pts)**. **SCORE** è la somma dei contributi sopra.")

            # ======= Score stabile + portfolio =======
            st.subheader("Riepilogo SEO STABILE")
            try:
            ...
            with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                ...
                try:
                    ...
                    pass
            st.download_button("📥 Scarica il report Excel (multi-file)", buffer.getvalue(), file_name="seo_audit_multi_v3.xlsx")
