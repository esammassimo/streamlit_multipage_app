...
# ===================== COSTANTI =====================
MAX_ROWS = 2000  # limite righe analizzabili per esecuzione
...
def normalize_domain(raw: str) -> str:
    ...
    Normalizza una stringa (URL o dominio) al dominio registrabile:
      ...
      - rimuove 'www.'
      - fallback: ultimi 2 label (es. blog.news.example.co.uk -> example.co.uk)
    NOTA: senza public suffix list, alcuni TLD 'particolari' (.co.jp, .com.au) potrebbero non essere perfetti.
    """
    if not isinstance(raw, str) or raw.strip() == "":
        return ""
    s = raw.strip()
    # se è un URL, estrai netloc
    if re.match(r"^[a-zA-Z]+://", s):
        netloc = urlparse(s).netloc
    else:
        # potrebbe essere già un dominio; togli eventuale path
        netloc = s.split("/")[0]
...
def backoff_sleep(attempt):
    # esponenziale semplice: 0.5, 1, 2, 4 (cap a 4s)
...
if input_method == "Carica file Excel":
    ...
    manual_input = st.text_area("✍️ Incolla domini o URL, uno per riga")
    if manual_input:
...
# Scelta del modello e batch size
        ...
        for idx, dom_batch in enumerate(batch(domains, batch_size), 1):
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    # parsing risposta: CSV semplice "dominio, categoria"
